const sheetURL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSbD1WEET8Z_X9SGBu_Nzfag8zO6NaBxnNn3HVBUH1erzlZAinL8oDIA1nUWGN07xd71HoR85fgQ2Yq/pub?output=csv";
...
function driveToImageURL(url) {
  ...
  const match = url.match(/\/d\/([^/]+)/) || url.match(/id=([^&]+)/) || url.match(/lh3\.googleusercontent\.com\/d\/([^/?]+)/);
  if (!match) return "";
...
Papa.parse(sheetURL, {
  ...
  complete: function(results) {
    ...
    wines = results.data.map(function(row) {
      const clean = {};
    ...
    if (totalCount) {
      ...
      console.log('Stats updated - Total:', wines.length);
    }
  ...
  error: function(error) {
    ...
    grid.innerHTML = '<div class="loading-state"><p>Failed to load collection. Please refresh the page.</p><p style="color: var(--text-tertiary); font-size: 0.875rem; margin-top: 1rem;">If this persists, check browser console for details.</p></div>';
  },
...
function buildFilterOptions() {
  const types = Array.from(new Set(wines.map(function(w) { return w.type; }).filter(Boolean))).sort();
  const regions = Array.from(new Set(wines.map(function(w) { return w.region; }).filter(Boolean))).sort();
  const grapes = Array.from(new Set(wines.map(function(w) { return w.grape; }).filter(Boolean))).sort();

  types.forEach(function(type) {
    const label = document.createElement('label');
    label.className = 'checkbox-label';
    label.innerHTML = '<input type="checkbox" class="type-filter" value="' + escapeHtml(type) + '"><span>' + escapeHtml(type) + '</span>';
  ...
  regions.forEach(function(region) {
    const label = document.createElement('label');
    label.className = 'checkbox-label';
    label.innerHTML = '<input type="checkbox" class="region-filter" value="' + escapeHtml(region) + '"><span>' + escapeHtml(region) + '</span>';
  ...
  grapes.forEach(function(grape) {
    const label = document.createElement('label');
    label.className = 'checkbox-label';
    label.innerHTML = '<input type="checkbox" class="grape-filter" value="' + escapeHtml(grape) + '"><span>' + escapeHtml(grape) + '</span>';
    grapeFilters.appendChild(label);
  });

  document.querySelectorAll('.type-filter, .region-filter, .grape-filter').forEach(function(input) {
...
function render(data) {
  ...
  const ratings = data.map(function(w) { return parseFloat(w.rating); }).filter(function(r) { return !isNaN(r) && r > 0; });
  if (avgRating) {
    if (ratings.length > 0) {
  ...
  if (data.length === 0) {
    grid.innerHTML = '<div class="loading-state"><p>No wines found matching your criteria.</p></div>';
    return;
  }

  grid.innerHTML = "";

  data.forEach(function(w, index) {
    const card = document.createElement("div");
    card.className = "wine-card";
    card.style.animationDelay = (index * 0.05) + "s";

    const imgSrc = driveToImageURL(w.image) || "https://via.placeholder.com/320x320/722f37/ffffff?text=No+Image";
    const vintage = w.vintage || "N/A";
    const type = w.type || "Unknown Type";

    if (currentView === 'grid') {
      card.innerHTML = '<img src="' + imgSrc + '" alt="' + escapeHtml(w.name) + ' Label" loading="lazy"><h3>' + escapeHtml(w.name) + '</h3><div class="meta">' + escapeHtml(vintage) + ' &middot; ' + escapeHtml(type) + '</div>';
    } else {
      card.innerHTML = '<img src="' + imgSrc + '" alt="' + escapeHtml(w.name) + ' Label" loading="lazy"><div class="card-content"><div class="card-info"><h3>' + escapeHtml(w.name) + '</h3><div class="meta">' + escapeHtml(vintage) + ' &middot; ' + escapeHtml(type) + '</div></div><div class="card-details"><div class="detail-chip"><span class="detail-chip-label">Region</span><span class="detail-chip-value">' + escapeHtml(w.region || 'N/A') + '</span></div><div class="detail-chip"><span class="detail-chip-label">Rating</span><span class="detail-chip-value">' + escapeHtml(w.rating || 'N/A') + '</span></div></div></div>';
    }
...
function openModal(index) {
  ...
  modalImg.src = driveToImageURL(w.image) || "https://via.placeholder.com/500x500/722f37/ffffff?text=No+Image";
...
prevWineBtn.addEventListener('click', function() {
  if (currentWineIndex > 0) {
    openModal(currentWineIndex - 1);
  }
});

nextWineBtn.addEventListener('click', function() {
...
function sortWines(data, field, direction) {
  ...
  return data.slice().sort(function(a, b) {
    ...
    if (["vintage", "rating", "price"].indexOf(field) !== -1) {
...
minRatingInput.addEventListener('input', function(e) {
...
function applyFilters() {
  ...
  const selectedRegions = Array.from(document.querySelectorAll('.region-filter:checked')).map(function(cb) { return cb.value; });
  const selectedGrapes = Array.from(document.querySelectorAll('.grape-filter:checked')).map(function(cb) { return cb.value; });
  ...
  let filtered = wines.filter(function(w) {
    if (term && Object.values(w).join(" ").toLowerCase().indexOf(term) === -1) {
      return false;
    }
    if (selectedTypes.length > 0 && selectedTypes.indexOf(w.type) === -1) {
      return false;
    }
    if (selectedRegions.length > 0 && selectedRegions.indexOf(w.region) === -1) {
      return false;
    }
    if (selectedGrapes.length > 0 && selectedGrapes.indexOf(w.grape) === -1) {
      return false;
    }
    ...
    if (buyYes.checked && !buyNo.checked) {
      if (!w.buy || w.buy.toLowerCase().indexOf('yes') === -1) {
        return false;
      }
    }
    if (buyNo.checked && !buyYes.checked) {
      if (!w.buy || w.buy.toLowerCase().indexOf('no') === -1) {
        return false;
      }
    }
...
exportBtn.addEventListener('click', function() {
  ...
  const rows = filteredWines.map(function(w) {
    return [
      w.name,
      w.vintage,
      w.type,
